...
import { ImageIcon, Sparkles, ArrowLeft } from 'lucide-react';
...
export default function OurProjectsGalleryPage() {
            ...
            <div className="max-w-7xl mx-auto relative z-10">
              ...
              <div className={`text-center mb-16 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
                ...
                <p
                  ...
                  style={{ fontFamily: 'Inter, sans-serif' }}
                >
                  Explore our complete collection of successful projects showcasing excellence across oil & gas, infrastructure, and energy sectors.
                </p>

                {/* Back to Home Button */}
                <Link href="/">
                  <button className="inline-flex items-center gap-2 px-6 py-3 bg-white/80 backdrop-blur-xl text-[#932445] font-semibold rounded-full shadow-lg hover:shadow-xl border-2 border-[#932445]/30 hover:border-[#932445]/50 transition-all duration-300 hover:scale-105">
              ...
              {/* Premium Projects Grid - All Images */}
              <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
                {ALL_PROJECT_IMAGES.map((project, index) => (
                  <div
                    key={project.id}
                  ...
                  >
                    ...
                    <div className="absolute bottom-4 left-4 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-2 group-hover:translate-y-0">
                      <div className="px-4 py-2 rounded-full bg-white/90 backdrop-blur-sm shadow-lg border-2 border-[#932445]/30">
                        <span className="text-[#932445] font-bold text-sm">#{String(project.id).padStart(2, '0')}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className={`mt-20 flex justify-center items-center gap-12 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '2s' }}>
